#include "DNSPTRServer.h"

static char *path = "ptrCache.txt";
...
int DNS_table_init(struct DNS_RR *answer, char *path, char *domain,unsigned short type)
{
...
int DNS_tcp()
{
    ...
    server_sockaddr.sin_addr.s_addr = inet_addr(DNS_PTR_ADDRESS);
    ...
    while (1)
    {
        ...
        int flag = DNS_table_init(answer, path, domain, type);
        if (flag == 1)
        {
            struct DNS_Header header = {0};
